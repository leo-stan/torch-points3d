...
class Visualizer(object):
        ...
        if self._stage in self._indices:
            ...
            for idx in np.argwhere(self._seen_batch == batch_indices).flatten():
                ...
                for visual_name, item in visuals.items():
                    ...
                    # if iterabledataset returns an incomplete batch
                    if len(out_item) == 0:
                        continue
    ...
    def save_las(self, out_path, out_item, label, fname):
        ...
        def unscale_rgb(val):
            ...
            min = -1
            max = 1
            return 65535 * (val - min) / (max - min)

        if "r" in out_item.dtype.names:
            pred_las.red = unscale_rgb(out_item["r"])
            pred_las.green = unscale_rgb(out_item["g"])
            pred_las.blue = unscale_rgb(out_item["b"])
